...
MDP.T = zeros(12,12,4); % Initialize transition matrix
...
%% Function to find the position of a state in the grid
...
for s = MDP.States
    if ~ismember(s, [5, 7, 12]) % States s5, s7, and s12 are terminal states
        ...
        MDP.T(s,grid(row,left),1) = 0.8;    % 80% probability of going "left"
        MDP.T(s,grid(row,right),3) = 0.8;   % 80% probability of going "right"
        MDP.T(s,grid(up,col),4) = 0.8;      % 80% probability of going "up"
        MDP.T(s,grid(down,col),2) = 0.8;    % 80% probability of going "down"

        % Adjusting transition probabilities at walls
        % If the agent would hit a wall, it remains in its current state
...
[v, pi] = Value_Iteration(MDP, gamma); 
